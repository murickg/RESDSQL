...
def parse_option():
    ...
    parser.add_argument('--table_path', type=str, default="./pauq/tables.json")
    parser.add_argument('--input_dataset_path', type=str, default="./pauq/pauq_train.json",
                        help='''
                            options:
    ...
    parser.add_argument('--output_dataset_path', type=str,
                        default="./pauq/preprocessed_data/preprocessed_pauq_train_natsql.json",
                        help="the filepath of preprocessed dataset.")
    parser.add_argument('--db_path', type=str, default="./pauq/merged_database_2022-06-10/merged_database",
...
def main(opt):
    ...
    for natsql_data, data in tqdm(zip(natsql_dataset, dataset)):
        ...
        preprocessed_data = {}
        preprocessed_data['final'] = data['final']
        preprocessed_data['db_id'] = db_id
        preprocessed_data['interaction'] = []
        for utterence in data['interaction']:
            if utterence[
                ...
                utterence['query_toks'] = ['SELECT', 'T1.company_type', 'FROM', 'Third_Party_Companies', 'AS', 'T1',
                                           'JOIN', 'Maintenance_Contracts', 'AS', 'T2', 'ON', 'T1.company_id', '=',
                                           'T2.maintenance_contract_company_id', 'ORDER', 'BY', 'T2.contract_end_date',
                                           'DESC', 'LIMIT', '1']
                utterence['query_toks_no_value'] = ['select', 't1', '.', 'company_type', 'from',
                ...
                utterence['utterance'] = 'What is the type of the company who concluded its contracts most recently?'
                utterence['utterance_toks'] = ['What', 'is', 'the', 'type', 'of', 'the', 'company', 'who', 'concluded',
                                               'its', 'contracts', 'most', 'recently', '?']
            if utterence['query'].startswith(
                    'SELECT T1.fname FROM student AS T1 JOIN lives_in AS T2 ON T1.stuid  =  T2.stuid WHERE T2.dormid IN'):
                utterence['query'] = utterence['query'].replace('IN (SELECT T2.dormid)', 'IN (SELECT T3.dormid)')
                index = utterence['query_toks'].index('(') + 2
                # assert data['query_toks'][index] == 'T2.dormid'
                utterence['query_toks'][index] = 'T3.dormid'
                index = utterence['query_toks_no_value'].index('(') + 2
                # assert data['query_toks_no_value'][index] == 't2'
                utterence['query_toks_no_value'][index] = 't3'

            question = utterence["utterance"].replace("\u2018", "'").replace("\u2019", "'").replace("\u201c",
                                                                                                    "'").replace(
                "\u201d", "'").strip()
            ...
            preprocessed_utterence = {}
            preprocessed_utterence["question"] = question

            preprocessed_utterence["sql"] = sql
            preprocessed_utterence["norm_sql"] = norm_sql
            preprocessed_utterence["sql_skeleton"] = sql_skeleton

            preprocessed_utterence["natsql"] = natsql
            preprocessed_utterence["norm_natsql"] = norm_natsql
            preprocessed_utterence["natsql_skeleton"] = natsql_skeleton

            preprocessed_utterence["db_schema"] = []
            preprocessed_utterence["pk"] = db_schemas[db_id]["pk"]
            preprocessed_utterence["fk"] = db_schemas[db_id]["fk"]
            preprocessed_utterence["table_labels"] = []
            preprocessed_utterence["column_labels"] = []
            ...
            for table in db_schemas[db_id]["schema_items"]:
                ...
                preprocessed_utterence["db_schema"].append({
                    "table_name_original": table["table_name_original"],
                ...
                if opt.target_type == "sql":
                    if table["table_name_original"] in sql_tokens:  # for used tables
                        preprocessed_utterence["table_labels"].append(1)
                        column_labels = []
                        for column_name_original in table["column_names_original"]:
                        ...
                        preprocessed_utterence["column_labels"].append(column_labels)
                    else:  # for unused tables and their columns
                        preprocessed_utterence["table_labels"].append(0)
                        preprocessed_utterence["column_labels"].append(
            ...
            preprocessed_data['interaction'].append(preprocessed_utterence)

...
class ColumnAndTableClassifierDataset(Dataset):
    def __init__(
        ...
        for interaction in dataset:
            for data in interaction['interaction']:
...
class CoSQLDataset(Dataset):
    def __init__(
        ...
        super(CoSQLDataset).__init__()

        self.mode = mode

        # TODO инициализация списков для вопросов, запросов и тд
        ...
        for dialoge in dataset:
            i = 0
            if self.mode == "train":
                for data in dialoge["interaction"]:
                    if i == 0:
                        self.db_ids.append(dialoge["db_id"])
                        self.input_sequences.append(f"U: | A: | {data['input_sequence']}")
                        self.output_sequences.append(data['output_sequence'])
                        self.all_tc_original.append(data['tc_original'])
                    else:
                        self.db_ids.append(dialoge["db_id"])
                        self.input_sequences.append(
                            f"U: {dialoge['interaction'][i - 1]['input_sequence']} | A: {dialoge['interaction'][i - 1]['output_sequence']} | U: {data['input_sequence']}")
                        self.output_sequences.append(data['output_sequence'])
                        self.all_tc_original.append(data['tc_original'])
                    i += 1

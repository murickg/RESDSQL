...
class EvaluateTool(object):
    ...
    def register_golds(self, dataset_filepath, db_path, predict_sqls):
        with open(dataset_filepath, encoding="utf-8") as f:
            ...
            for idx, sample in enumerate(dataset):
                if sample['query'] == 'SELECT T1.company_name FROM Third_Party_Companies AS T1 JOIN Maintenance_Contracts AS T2 ON T1.company_id  =  T2.maintenance_contract_company_id JOIN Ref_Company_Types AS T3 ON T1.company_type_code  =  T3.company_type_code ORDER BY T2.contract_end_date DESC LIMIT 1':
                    ...
                    sample['input'] = 'What is the type of the company who concluded its contracts most recently?'
                ...
                if sample['label'] == "CONFIRM_SQL" and predict_sqls[idx].split(":")[0].strip() == "CONFIRM_SQL":
                    self.predicted.append(predict_sqls[idx].split(":")[-1].strip())
                    self.golds.append({
                        "query": sample["query"],
    ...
    def evaluate(self, preds):
        exact_match = compute_exact_match_metric(self.predicted, self.golds)
        test_suite = compute_test_suite_metric(self.predicted, self.golds, db_dir=None)

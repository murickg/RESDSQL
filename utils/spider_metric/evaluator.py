...
class EvaluateTool(object):
    ...
    def register_golds(self, dataset_filepath, db_path):
        with open(dataset_filepath, encoding="utf-8") as f:
            dataset = json.load(f)
            for idx, dialoge in enumerate(dataset):
                for sample in dialoge["interaction"]:
                    if sample['query'] == 'SELECT T1.company_name FROM Third_Party_Companies AS T1 JOIN Maintenance_Contracts AS T2 ON T1.company_id  =  T2.maintenance_contract_company_id JOIN Ref_Company_Types AS T3 ON T1.company_type_code  =  T3.company_type_code ORDER BY T2.contract_end_date DESC LIMIT 1':
                        ...
                        sample['query_toks_no_value'] = ['select', 't1', '.', 'company_type', 'from',
                                                         ...
                                                         'maintenance_contracts', 'as', 't2', 'on', 't1', '.',
                                                         'company_id', '=', 't2', '.',
                                                         'maintenance_contract_company_id', 'order', 'by', 't2', '.',
                                                         'contract_end_date', 'desc', 'limit', 'value']
                        sample['utterance'] = 'What is the type of the company who concluded its contracts most recently?'
                        sample['utterance_toks'] = ['What', 'is', 'the', 'type', 'of', 'the', 'company', 'who',
                                                    'concluded', 'its', 'contracts', 'most', 'recently', '?']
